...
for iteration in range(300):
    ...
    for cnt in range(0,len(training_set_outputs)):
        err[cnt]= training_set_outputs[cnt] - output[cnt]
        #err=training_set_outputs - output

    delta_v=[0, 0, 0, 0]
    for cnt in range(0,len(training_set_outputs)):
        delta_v[cnt]= (err[cnt]) * output[cnt] * (1 - output[cnt])

    for cnt in range(0,len(training_set_outputs)):
        sumweights=0
        for cnin in range(0,numinputs):
            synaptic_weights[cnin]=synaptic_weights[cnin]+training_set_inputs[cnt][cnin]*delta_v[cnt]
